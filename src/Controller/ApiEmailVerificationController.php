...
class ApiEmailVerificationController extends AbstractController
{
    ...
    /**
     * Verify email with token (POST JSON — mobile or API clients).
     */
    ...
    /**
     * Resend verification email by address (no login required — for mobile sign-up flow).
     */
    ...
    public function resendVerificationByEmail(Request $request): JsonResponse
    {
        ...
        if ($user->isVerified()) {
            return $this->json([
                'success' => true,
                'message' => 'This email is already verified. You can log in on the app.',
            ], 200);
        }
        ...
        return $this->json([
            'success' => true,
            'message' => 'Verification email sent. Open the link on any device, then log in on the app.',
        ], 200);
    }
